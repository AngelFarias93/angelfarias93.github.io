...
define("CONTROLADOR_DEFECTO", "plantilla");
define("ACCION_DEFECTO", "ctrPlantilla");
define("VERSIONRECURSOS", "1.1");
define("IDRECURSOS", clave_digit(10));
/*=============================================
DATOS DEL SISTEMA
=============================================*/
define("NAMESOFTWARE", "Consultoría en Psicología");
define("ICONO", $_ENV["RUTA"] . "public/img/icono1.png");
define("ESTILOS_ICONOS", "width:40px;height:40px;");
define("NUMERO_PSICOLOGA", "+52 55-5555-5555");
define("CORREO_PSICOLOGA", "ejemplo@correo.com");
define("ADDRESS_PSICOLOGA", "De La Trinidad 172, Residencial San Angel, León, Gto.");
define("VERSION", VERSIONRECURSOS . IDRECURSOS);
/*=============================================
DATOS DE TECSOLT
=============================================*/
define("CORREO_TECSOLT", "info@tecsolt.com");
define("LOGOTECSOLT", $_ENV["RUTA"]."public/img/tecsolt_sin_fondo.png");
define("LINKFACEBOOK", "https://www.facebook.com/TecsoltOficial/");
define("LINKWEB", "https://www.tecsolt-company.com/");
define("LINKWEB2", "https://www.tecsolt.com/");
/*=============================================
VISTAS DEL SISTEMA
=============================================*/
define("BUILD_VIEW", '<div class="col-sm-12 col-md-12 col-lg-12">
<style>
    .titleSite{
        font-weight:100;
        font-size:27pt;
...
<div class="warning-content">
    <h1 class="titleSite font-weight-bold">Nueva vista en construción</h1>
    ...
    <p class="text-body text-muted">
        ...
        Estamos trabajando lo más rápido posible y cuando este listo nosotros te avisaremos para que puedas hacer el rastreo de tus ordenes.
    </p>
</div>
</div>');
/*=============================================
...
=============================================*/
/********AVISO DE APAGADO********/
$inicio = 8; # Desde las 08:00 am .
$fin = 20; # Hasta las 20:00 pm.
$HoraActual = intval(date("H"));

if ($HoraActual >= $inicio && $HoraActual < $fin) {
    define("MSG_TIME", '');
} else {
    define("MSG_TIME", '<div class="alert alert-warning mb-3">
                            <h4><i class="fas fa-bell mr-2"></i>Notificación de Mantenimiento</h4>
                            <p>Recuerda que por mantenimiento del servidor el sistema dejara de estar disponible a las <b>9:00 pm</b>, por lo tanto <u>no podrás
                            ingresar hasta las 8:00 am del día siguiente</u>.</p>
                        </div>');
}

//VALUE REGEX
define("REGEX_GENERAL", "\(\)\_\-\/ a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ $@$#!%*?&.");
define("REGEX_GENERAL1", "\:\;\"\(\)\_\-\/ a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ $@$#!%*?&.,:;");
define("REGEX_CARAC_ESP_1", "\(\)\_\-\/ ");
define("REGEX_CARAC_ESP_2", " $@$#!%*?&.");
define("REGEX_TEXTO1", "a-zA-Z0-9 ñÑáéíóúÁÉÍÓÚ,.");
define("REGEX_TEXTO2", "a-zA-Z ñÑáéíóúÁÉÍÓÚ,.");
define("REGEX_TEXTO3", "a-zA-Z ");
define("REGEX_TEXTO4", "a-zA-Z0-9");
define("REGEX_NUMERO", "0-9 ");
define("REGEX_TELEFONO", "-\+\/\ 0-9");
define("REGEX_NUMCARAC_10", "1,10");
define("REGEX_NUMCARAC_25", "1,25");
define("REGEX_NUMCARAC_50", "1,50");
define("REGEX_NUMCARAC_75", "1,75");
define("REGEX_NUMCARAC_100", "1,100");
//VALUE GENERAL
define("OPTIONAL", "<small class='ml-1 text-muted'>(Opcional)</small>");
define("REQUIRED", "<span class='text-red-50 ml-1'>*</span>");
/*=============================================================
CREAMOS METODO PARA UNA ALERTA MODAL GENERICA DE TIPO SWAL
=============================================================*/
function walL_method_views($type_swal, $value_type_icon, $title, $text, $mensaje, $redirect)
{
    $ruta = $_ENV["RUTA"] . $redirect;

    if ($redirect == null){
        $ruta = "";
    }else{
        $ruta = "window.location = '".$_ENV["RUTA"].$redirect."'";
    }

    if ($type_swal == "Swal.fire") {
        echo "<script>
                Swal.fire({
...
function walL_method_views_without_return($type_swal, $value_type_icon, $title, $text, $mensaje, $redirect)
{
    $ruta = $_ENV["RUTA"] . $redirect;
 ...
 */
function toredirect($valor)
{
    echo '<script>window.location="' . $_ENV["RUTA"] . $valor . '"</script>';
}
/*
 * Función para sacar los dias del mes
 */
function diasMes($formatAnio = null, $formatMes = null)
{
    if(is_null($formatAnio)) {
        $fa = "Y"; 
    } 
    else {
        $fa = $formatAnio;
    } 
    if(is_null($formatMes)) {
        $fm = "m"; 
    } 
    else {
        $fm = $formatMes;
 ...
 * 
 */
function clave_digit($long)
{
    //Se define una cadena de caractares.
    //Los recomiendo desordenar las minúsculas, mayúsculas y números para mejorar la probabilidad.
    $cadena = "1234567890";
